(function () {
  ...
  function Eventer() {
     ...
     * @return {{ listener: Function, _this: Any }}
     */
     ...
     * @param {Function} listener The function to be called. Defaults to noop
     * @param {Any} _this Other identification to use for when the listener could be anonymous
     * @return {{ listener: Function, _this: Any, $isOnce: Boolean }}
     */
    this.once = function (eventName, listener, _this) {
      if (typeof listener !== 'function') {
        listener = noop;
      }

      var eventItem = { listener: listener, _this: _this, $isOnce: true };
    ...
    this.clearAll = function () {
      Object.keys(this.$events).forEach(this.clear);
    }.bind(this);
    ...
    this.trigger = function () {
      ...
      // Call all listeners on *eventName* if there are any
      if (0 < listeners.length) {
        listeners.forEach(function (eventItem) { return eventItem.listener.apply(eventItem.this, args.slice(1)); });

        // Remove listeners from all items marked as $isOnce
        listeners.forEach(function (eventItem) {
          if (eventItem.$isOnce) {
            this.off(eventName, eventItem.listener, eventItem._this);
          }
        }.bind(this));
      }
    }.bind(this);
